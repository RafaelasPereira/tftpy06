...
def get_file(server_addr: INET4Address, filename: str, destination: None):
    """
    Get the remote file given by `filename` through a TFTP RRQ
    connection to remote server at `server_addr` 
    and save it to local path `destination` (if filled).
    """
    with socket(AF_INET, SOCK_DGRAM) as sock:
        sock.settimeout(INACTIVITY_TIMEOUT)
        with open(destination or filename, 'wb') as file:
